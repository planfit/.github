...
`@v1`은 이동 태그 → 강제 푸시 시 **전 레포 동시**. 따라서:

1. feature 브랜치 개발 → `@v2-beta` 태그(또는 SHA 핀).
2. **canary**: planfit-client-rn caller만 `@<sha>`로 잠시 핀 → 실제 PR 몇 건 검증.
3. 통과 시 `git tag -f v1 && git push origin v1 --force`.
4. 롤백: 직전 SHA로 `v1` 재태깅.

**안전순서**: L2(opt-in, 무위험) 먼저 → L3(Step6, 전 레포 synchronize 동작 변경) canary 후 → 학습루프 → decay.
